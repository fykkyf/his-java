...
public interface ClinicOrderMapper {
    ...
    @Select("select treatment_price from treatment where treatment_id=#{treatmentId} and treatment_status=1")
    Double getPriceByTreatmentId(ClinicOrder clinicOrder);

    @Insert("insert into clinic_lab values (null,#{visitor_id},#{treatment_id},null,now(),1)")
    //下医嘱时，如果需要进行检验，则在门诊检验表中生成数据
    void addClinicLab(ClinicOrder clinicOrder);

    @Insert("insert into clinic_raidology values (null,#{visitor_id},#{treatment_id},null,now(),1)")
